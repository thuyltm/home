...
func servePage(w http.ResponseWriter, r *http.Request) {
	...
	t, err := template.ParseFiles("magnet/go4vl/webcam/webcam.html")
	if err != nil {
...
func serveVideoStream(w http.ResponseWriter, req *http.Request) {
	...
	w.Header().Set("Content-Type",
		fmt.Sprintf("multipart/x-mixed-replace;boundary=%s", mimeWriter.Boundary()))
...
func initFaceDetect() error {
	model, err := os.ReadFile("magnet/go4vl/webcam/facefinder.model")
	if err != nil {
		return fmt.Errorf("faled to load face finder model: %s", err)
	}
	p := pigo.NewPigo()
...
func runFaceDetect(partWriter io.Writer, frame []byte) any {
	...
	return drawer.EncodePNG(partWriter)
}
...
func controlVideo(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		log.Printf("falied to set brightness: %s", err)
		return
	}
	switch ctrl.Name {
	case "brightness":
...
func main() {
	...
	flag.StringVar(&format, "f", format, "pizel format")
	...
	camera, err = device.Open(devName,
		...
		device.WithPixFormat(v4l2.PixFormat{PixelFormat: getFormatType(format), Width: uint32(width),
			Height: uint32(height), Field: v4l2.FieldAny}),
	...
	streamInfo = fmt.Sprintf("%s - %s [%dx%d] %d fps",
		caps.Card, v4l2.PixelFormats[currFmt.PixelFormat], currFmt.Width, currFmt.Height, frameRate)
	//start capture
	...
	//setup http service
	http.HandleFunc("/webcam", servePage)
	http.HandleFunc("/stream", serveVideoStream)
	http.HandleFunc("/control", controlVideo)
	http.HandleFunc("/face", faceDetectControl)
